...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate hybrid forecast with a separately learned range model."
    )
...
def _build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["pred_up_share"] = np.where(out["pred_range"] > 1e-9, out["pred_up_ext"] / out["pred_range"], 0.5)
    out["pred_down_share"] = np.where(out["pred_range"] > 1e-9, out["pred_down_ext"] / out["pred_range"], 0.5)
    ...
    lag_cols = [
        ...
        "pred_gap_abs", "pred_close_from_open_abs",
        "actual_gap_abs", "actual_close_from_open_abs",
    ]
    for col in lag_cols:
...
def _fit_range_model(train: pd.DataFrame, feature_cols: list[str]) -> LGBMRegressor:
    model = LGBMRegressor(
    ...
    model.fit(train[feature_cols], train["actual_range"])
    return model


def _apply_range_model(df: pd.DataFrame, model: LGBMRegressor, feature_cols: list[str]) -> pd.DataFrame:
    out = df.copy()

    out["range_model_pred"] = model.predict(out[feature_cols])
    out["range_model_pred"] = np.maximum(out["range_model_pred"], 0.0)

    # Use hybrid's asymmetry shares
    out["rm_pred_open"] = out["pred_open"]
    out["rm_pred_close"] = out["pred_close"]
    out["rm_pred_high"] = out["pred_open"] + out["range_model_pred"] * out["pred_up_share"]
    out["rm_pred_low"] = out["pred_open"] - out["range_model_pred"] * out["pred_down_share"]
...
def _monthly_compare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for month, g in tmp.groupby("month"):
        ...
        for k, v in rm.items():
            row[f"rm_{k}"] = v
        for k in orig.keys():
            row[f"diff_{k}_rm_minus_orig"] = rm[k] - orig[k]
        rows.append(row)
...
def main() -> None:
    ...
    model = _fit_range_model(train_df, feature_cols)
    evaluated = _apply_range_model(test_df, model, feature_cols)

    orig_metrics = _metric_bundle(evaluated, "pred")
    rm_metrics = _metric_bundle(evaluated, "rm_pred")
    diff = {k: rm_metrics[k] - orig_metrics[k] for k in orig_metrics.keys()}
    ...
    pred_cols = [
        "forecast_date", "prev_close",
        "pred_open", "pred_high", "pred_low", "pred_close",
        "rm_pred_open", "rm_pred_high", "rm_pred_low", "rm_pred_close",
        "actual_open", "actual_high", "actual_low", "actual_close",
        "pred_range", "actual_range", "range_model_pred",
        "pred_up_share", "pred_down_share",
    ]
    ...
    report = {
        ...
        "range_model_metrics": rm_metrics,
        "metric_diff_range_model_minus_original": diff,
    }

    summary_path = outdir / "hybrid_plus_range_model_summary.json"
    monthly_path = outdir / "hybrid_plus_range_model_monthly.csv"
    preds_path = outdir / "hybrid_plus_range_model_predictions.csv"
    ...
    print()
    print("=== HYBRID PLUS RANGE MODEL CHECK ===")
    ...
    for k, v in report["original_metrics"].items():
        print(" ", k, v)
    print()
    print("Range-model metrics:")
    for k, v in report["range_model_metrics"].items():
        print(" ", k, v)
    print()
    print("Diff (range-model - original):")
    for k, v in report["metric_diff_range_model_minus_original"].items():
        print(" ", k, v)
